...
Migration.create = function(name, directory){
  ...
  var suffix = argv.coffee ? 'coffee' : 'js'
  var new_file = util.format('%s/%d_%s.%s', directory, +new Date(), name, suffix);

  var template = 'migration-template.' + suffix;

  // copy the default migration template to the new file location
  fs.createReadStream( path.resolve(__dirname, template))
